...
# BiblesInternational/Makefile.  Generated from Makefile.in by configure.
...
# Installing the cross-reference database that is needed for the Bibledit Analysis window
...
am__installdirs = "$(DESTDIR)$(crossref_dir)"
DATA = $(crossref__DATA)
...
XML_LIBS = -lxml2
abs_builddir = /home/postiffm/bibledit/bibledit-desktop/BiblesInternational
abs_srcdir = /home/postiffm/bibledit/bibledit-desktop/BiblesInternational
...
crossref_dir = $(pkgdatadir)/bibles

# Files to install
crossref__DATA = bi.crf open_bible_info.crf repeated_phrases.rpf
CLEANFILES = *~
all: all-am
...
$(srcdir)/Makefile.in:  $(srcdir)/Makefile.am  $(am__configure_deps)
	...
	echo ' cd $(top_srcdir) && $(AUTOMAKE) --gnu BiblesInternational/Makefile'; \
	$(am__cd) $(top_srcdir) && \
	  $(AUTOMAKE) --gnu BiblesInternational/Makefile
...
clean-libtool:
	-rm -rf .libs _libs
install-crossref_DATA: $(crossref__DATA)
	@$(NORMAL_INSTALL)
	@list='$(crossref__DATA)'; test -n "$(crossref_dir)" || list=; \
	if test -n "$$list"; then \
	  echo " $(MKDIR_P) '$(DESTDIR)$(crossref_dir)'"; \
	  $(MKDIR_P) "$(DESTDIR)$(crossref_dir)" || exit 1; \
	fi; \
	for p in $$list; do \
	...
	while read files; do \
	  echo " $(INSTALL_DATA) $$files '$(DESTDIR)$(crossref_dir)'"; \
	  $(INSTALL_DATA) $$files "$(DESTDIR)$(crossref_dir)" || exit $$?; \
	done

uninstall-crossref_DATA:
	@$(NORMAL_UNINSTALL)
	@list='$(crossref__DATA)'; test -n "$(crossref_dir)" || list=; \
	files=`for p in $$list; do echo $$p; done | sed -e 's|^.*/||'`; \
	dir='$(DESTDIR)$(crossref_dir)'; $(am__uninstall_files_from_dir)
tags TAGS:
...
installdirs:
	for dir in "$(DESTDIR)$(crossref_dir)"; do \
...
install-data-am: install-crossref_DATA
...
uninstall-am: uninstall-crossref_DATA
...
.PHONY: all all-am check check-am clean clean-generic clean-libtool \
	...
	install install-am install-crossref_DATA install-data \
